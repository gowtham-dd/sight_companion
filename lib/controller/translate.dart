...
class TranslaterController extends GetxController {
  ...
  List charList = [].obs;
  List showsight_companion = [].obs;
  var text = "".obs;
  static const List<Map<String, String>> handsight_companionMap = [
  ...
  void TranslateToSign() {
    ...
    showsight_companion = List.filled(charList.length, null);
    for (int i = 0; i < charList.length; i++) {
      showsight_companion[i] = handsight_companionMap
          .where((element) =>
              element.containsKey(charList[i].toString().toUpperCase()))
          .first;
    }
    print(showsight_companion.length);
    update();
  }
