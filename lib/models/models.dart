...
enum GameStates { running, won, pause, initial }
...
class GameState extends ChangeNotifier {
  ...
  static const String defaultGoalText =
      "Write the text of this field, letter by letter, by turning on the right dots.";
  ...
  bool isRightAnswer(BrailleAsBoolList input) {
    return (input ==
            (Braille.charToBrailleBoolList(targetCharacter) ??
                BrailleAsBoolList()) &&
        (state != GameStates.won));
  }
...
void submitButtonPress(BuildContext context) {
  if (Provider.of<GameState>(context, listen: false)
      .submitInput(Provider.of<InputState>(context, listen: false).keys)) {
  ...
  Provider.of<InputState>(context, listen: false)
      .changeToALetter(BrailleAsBoolList());
}
...
class InputState extends ChangeNotifier {
  ...
  changeDot(BuildContext context, int index, {bool? state}) {
    //state == null =>  means to toggle state
